...
export class AuthService {
  ...
  isUserLogged : BehaviorSubject<boolean> = new BehaviorSubject(!!this.localStorageService.getJwtToken());
  isAdmin : BehaviorSubject<boolean> = new BehaviorSubject(this.localStorageService.getUser()?.role === Roles.Admin);
  ...
  public register(data: RegisterRequestInterface): Observable<Guid>{
    ...
    return this.http
      .post<Guid>(url, data.user).pipe(map(response => {return response}));
  }
  ...
  public login(data: LoginRequestInterface): Observable<LoginResponseInterface>{
    ...
    return this.http
      .post<LoginResponseInterface>(url, data.user)
      .pipe(map(response =>{
  ...
  public refreshToken(data: RefreshTokenInterface) : Observable<string>{
    ...
    return this.http.post<TokensInterface>(url, data).
    pipe(map(response =>{
      const jwtToken = response.jwtToken;
      const refreshToken = response.refreshToken;
      this.localStorageService.setJwtToken(jwtToken);
